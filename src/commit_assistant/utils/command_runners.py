...
class GitCommandRunner(CommandRunner):
    ...
    def validate_repo(self) -> None:
        ...
        if not git_dir.exists():
            raise ValueError(f"路徑 {self.repo_path} 不是有效的 git 倉庫（找不到.git 資料夾）")

        console.print(f"[green] 成功找到 git 倉庫：{self.repo_path}[/green]")
